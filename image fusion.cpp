...
void Gaussin_filter_up(double *a, double* c, int ori_row, int ori_col, double *b)//高斯滤波
{
    int i, j;
...
void up_sample_even(double* XF, double* up_sample, int filter_rows, int filter_cols)//向下采样，去掉偶数行和列
{
    int m, n;
    int filter_r0 = 0;//初始化
    int filter_c0 = 0;
...
int main()
{
    ...
    int colNumber = img.cols * 3;// img.channels();   //列数*通道数=每一行元素的个数

    for (int i = 0; i < img.rows; i++)  //行循环，可根据需要换成rowNumber
    {
        uchar* data = img.ptr<uchar>(i);  //获取第i行的首地址,data是指向第一个数值的指针，类型为uchar。
    ...
    //第一层高斯金字塔
    Gaussin_filter_up(B, BF, row, col, b);//高斯滤波
    ...
    //第二层高斯金字塔
    Gaussin_filter_up(up_sample1_1, BF, row/2, col/2, b);//下采样，去掉偶数行和列
    ...
    //第零层拉普拉斯金字塔
    down_sample(down_sample1_1, up_sample1_1, row, col, row / 2, col / 2);//上采样，偶数行和列补0
    down_sample(down_sample1_2, up_sample1_2, row, col, row / 2, col / 2);
    down_sample(down_sample1_3, up_sample1_3, row, col, row / 2, col / 2);
    Gaussin_filter_down(down_sample1_1, down_sample_filter1_1, row, col, b);//高斯滤波
    ...
    lp_pyrmid(B, down_sample_filter1_1, row, col, lp1_1);//生成拉普拉斯金字塔
    lp_pyrmid(G, down_sample_filter1_2, row, col, lp1_2);
    lp_pyrmid(R, down_sample_filter1_3, row, col, lp1_3);
    //第一层拉普拉斯金字塔
    down_sample(down_sample2_1, up_sample2_1, row/2, col/2, row / 4, col / 4);//上采样，偶数行和列补0
    ...
    Gaussin_filter_down(down_sample2_1, down_sample_filter2_1, row/2, col/2, b);//高斯滤波
    ...
    lp_pyrmid(up_sample1_1, down_sample_filter2_1, row/2, col/2, lp2_1);//生成拉普拉斯金字塔
    ...
    //第二层拉普拉斯金字塔
    down_sample(down_sample3_1, up_sample3_1, row /4, col / 4, row / 8, col / 8);//上采样，偶数行和列补0
    ...
    Gaussin_filter_down(down_sample3_1, down_sample_filter3_1, row / 4, col / 4, b);//高斯滤波
    ...
    lp_pyrmid(up_sample2_1, down_sample_filter3_1, row / 4, col / 4, lp3_1);//生成拉普拉斯金字塔
    ...
    int rowNumber2 = img2.rows;    //行数
    int colNumber2 = img2.cols * 3;// img.channels();   //列数*通道数=每一行元素的个数

    for (int i = 0; i < img2.rows; i++)  //行循环，可根据需要换成rowNumber
    {
        uchar* data2 = img2.ptr<uchar>(i);  //获取第i行的首地址,data是指向第一个数值的指针，类型为uchar。
        for (int j = 0; j < colNumber2; j++)  //列循环，同理
        {
    ...
    //第一层高斯金字塔
    Gaussin_filter_up(B2, BF2, row, col, b2);//高斯滤波
    ...
    //第二层高斯金字塔
    Gaussin_filter_up(up_sample1_1_2, BF2, row / 2, col / 2, b2);//下采样，去掉偶数行和列
    ...
    //第零层拉普拉斯金字塔
    down_sample(down_sample1_1_2, up_sample1_1_2, row, col, row / 2, col / 2);//上采样，偶数行和列补0
    ...
    Gaussin_filter_down(down_sample1_1_2, down_sample_filter1_1_2, row, col, b2);//高斯滤波
    ...
    lp_pyrmid(B2, down_sample_filter1_1_2, row, col,lp1_1_2);//生成拉普拉斯金字塔
    lp_pyrmid(G2, down_sample_filter1_2_2, row, col, lp1_2_2);
    lp_pyrmid(R2, down_sample_filter1_3_2, row, col, lp1_3_2);
    //第一层拉普拉斯金字塔
    down_sample(down_sample2_1_2, up_sample2_1_2, row / 2, col / 2, row / 4, col / 4);//上采样，偶数行和列补0
    ...
    Gaussin_filter_down(down_sample2_1_2, down_sample_filter2_1_2, row / 2, col / 2, b2);//高斯滤波
    ...
    lp_pyrmid(up_sample1_1_2, down_sample_filter2_1_2, row / 2, col / 2, lp2_1_2);//生成拉普拉斯金字塔
    ...
    //第二层拉普拉斯金字塔
    down_sample(down_sample3_1_2, up_sample3_1_2, row / 4, col / 4, row / 8, col / 8);//上采样，偶数行和列补0
    ...
    Gaussin_filter_down(down_sample3_1_2, down_sample_filter3_1_2, row / 4, col / 4, b2);//高斯滤波
    ...
    lp_pyrmid(up_sample2_1_2, down_sample_filter3_1_2, row / 4, col / 4, lp3_1_2);//生成拉普拉斯金字塔
